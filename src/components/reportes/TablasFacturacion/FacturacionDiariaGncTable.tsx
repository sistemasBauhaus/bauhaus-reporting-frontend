...
import { fetchFacturacionDiariaGNC, FacturacionDiariaGNC } from '../../../api/facturacion';
import { LinearProgress } from '@mui/material';


const FacturacionDiariaGncTable: React.FC = () => {
  const today = new Date();
  const year = today.getFullYear();
  const month = today.getMonth();
  const daysInMonth = new Date(year, month + 1, 0).getDate();
  const currentDay = today.getDate();
  const [data, setData] = useState<FacturacionDiariaGNC[]>([]);
  ...
  useEffect(() => {
    fetchFacturacionDiariaGNC()
      .then((res: FacturacionDiariaGNC[]) => {
        setData(res);
        setLoading(false);
      })
      .catch((err: any) => {
        setError('Error al obtener la facturación diaria de GNC');
        setLoading(false);
      });
  }, []);
  ...
  // Calcular total acumulado y proyectado después de filtrar
  const dataMesActual = filteredData.filter(row => {
    if (!row.fecha) return false;
    const fechaRow = new Date(row.fecha);
    return fechaRow.getMonth() === month && fechaRow.getFullYear() === year;
  });

  const totalVentasMes = dataMesActual.reduce((acc, row) => acc + Number(row.total_gnc_dinero), 0);
          ...
          <Table sx={{ minWidth: 700 }}>
            <TableHead>
              <TableRow sx={{ backgroundColor: '#e3f2fd' }}>
                ...
                <TableCell sx={{ fontWeight: 'bold', color: '#0d47a1' }}>GNC (m³)</TableCell>
                <TableCell sx={{ fontWeight: 'bold', color: '#0d47a1' }}>GNC AC (m³)</TableCell>
                <TableCell sx={{ fontWeight: 'bold', color: '#0d47a1' }}>Total GNC Dinero ($)</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {paginatedData.map((row, idx) => (
                <TableRow key={idx} hover sx={{ backgroundColor: idx % 2 === 0 ? '#fff' : '#e3f2fd' }}>
                  ...
                  <TableCell>{row.gnc !== undefined ? Number(row.gnc).toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '0.00'}</TableCell>
                  <TableCell>{row.gnc_ac !== undefined ? Number(row.gnc_ac).toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '0.00'}</TableCell>
                  <TableCell>{row.total_gnc_dinero !== undefined ? Number(row.total_gnc_dinero).toLocaleString('es-AR', { style: 'currency', currency: 'ARS', minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '$0.00'}</TableCell>
                </TableRow>
              ))}
              {/* Totales al pie */}
              <TableRow sx={{ backgroundColor: '#0d47a1' }}>
                ...
                <TableCell sx={{ fontWeight: 'bold', color: '#fff' }}>
                  {filteredData.length > 0 ?
                    filteredData.reduce((acc, row) => acc + Number(row.gnc), 0).toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })
                    : '0.00'}
                </TableCell>
                <TableCell sx={{ fontWeight: 'bold', color: '#fff' }}>
                  {filteredData.length > 0 ?
                    filteredData.reduce((acc, row) => acc + Number(row.gnc_ac), 0).toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })
                    : '0.00'}
                </TableCell>
                <TableCell sx={{ fontWeight: 'bold', color: '#fff' }}>
                  {filteredData.length > 0 ?
                    filteredData.reduce((acc, row) => acc + Number(row.total_gnc_dinero), 0).toLocaleString('es-AR', { style: 'currency', currency: 'ARS', minimumFractionDigits: 2, maximumFractionDigits: 2 })
                    : '$0.00'}
                </TableCell>
              </TableRow>
            </TableBody>
...
export default FacturacionDiariaGncTable;
